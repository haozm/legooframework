...
public class EmployeeServer extends AbstractBaseServer {
	...
	public Optional<List<EmployeeEntity>> loadEnabledShoppingGuides(Integer storeId, LoginUserContext userContext) {
		...
		Preconditions.checkState(
				...
				"门店与登陆用户公司不一致.");
		Optional<List<EmployeeEntity>> emploies = getBean(EmployeeEntityAction.class).loadEmployeesByStore(store.get(),
				null);
		if (!emploies.isPresent())
			return Optional.absent();
		List<EmployeeEntity> shoppingGuides = emploies.get().stream().filter(EmployeeEntity::isEnabled)
	...
	public void saveStoreEmployee(LoginUserContext loginUser, Integer storeId, String loginName, String userName,
		...
		getBean(EmployeeEntityAction.class).saveStoreEmployee(loginUser, store, loginName, userName, phoneNo, sex,
				roles);
	}
	...
	private void checkOrgEmpRoles(Collection<RoleEntity> roles) {
		...
		for (RoleEntity role : roles) {
			if (!(role.isManager() || role.isBoss()))
				isRigthRoles = false;
		}
		Preconditions.checkState(isRigthRoles, "组织职员角色只能是【经理】或【总经理】");
	}
	 ...
	 * @param roleIds
	 */
	public void editOrgEmployee(LoginUserContext loginUser, Integer orgId, Integer empId, String userName,
			String phoneNo, int sex, Collection<Integer> roleIds) {
		Preconditions.checkNotNull(loginUser, "入参loginUser不能为空");
	 ...
	 * @param roleIds
	 */
	public void editStoreEmployee(LoginUserContext loginUser, Integer storeId, Integer empId, String userName,
			String phoneNo, int sex, Collection<Integer> roleIds) {
		Preconditions.checkNotNull(loginUser, "入参loginUser不能为空");
		Preconditions.checkNotNull(empId, "入参empId不能为空");
		checkStoreRole(loginUser);
	 ...
	 * @param empIds
	 */
	public void fireEmployees(Integer storeId, List<Integer> empIds) {
		Preconditions.checkNotNull(storeId, "入参storeId不能为空");
		if (CollectionUtils.isEmpty(empIds))
			return;
		StoreEntity store = getBean(StoreEntityAction.class).loadById(storeId);
		getBean(EmployeeEntityAction.class).fireEmployees(store, empIds);
	}
	/**
	 * 迁移职员
	 * @param loginUser
	 ...
	 * @param empIds
	 */
	public void switchStore(LoginUserContext loginUser, Integer sourceStoreId, Integer destStoreId,
			Integer empId) {
		Preconditions.checkNotNull(loginUser);
		Preconditions.checkNotNull(sourceStoreId, "入参sourceStoreId不能为空");
		Preconditions.checkNotNull(sourceStoreId, "入参destStoreId不能为空");
		Preconditions.checkNotNull(empId, "入参empId不能为空");
		StoreEntity sourceStore = getBean(StoreEntityAction.class).loadById(sourceStoreId);
		StoreEntity destStore = getBean(StoreEntityAction.class).loadById(destStoreId);
		getBean(EmployeeEntityAction.class).switchStore(loginUser, sourceStore, destStore, empId);
	}
