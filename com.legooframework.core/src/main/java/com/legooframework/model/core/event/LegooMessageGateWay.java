...
public class LegooMessageGateWay implements MessageGateWay, ApplicationContextAware {
     ...
     */
    @Override
    public void postEvent(String channel, LegooEvent event) {
        LoginContext loginContext = LoginContextHolder.get();
        if (logger.isDebugEnabled())
            logger.debug(String.format("postEvent(%s,%s)", channel, event));
        template.send(channel, event.toMessage(loginContext));
    }

    @Override
    public void postEvent(LegooEvent event) {
        this.postEvent(SYNC_EVENTBUS, event);
    }
