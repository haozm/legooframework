...
public class BundleService extends BaseService {
     ...
     * @return 发送结果
     */
    public String send(SMSChannel channel, String mobile, String content, long smsExt, boolean encoded) {
        if (logger.isDebugEnabled())
            logger.debug(String.format("send(SMSChannel=%d, mobile=%s, smsExt=%d) start...",
                    channel.getChannel(), mobile, smsExt));
        SMSSubAccountEntity account = getBean(SMSProviderEntityAction.class).loadSubAccountByChannel(channel);
