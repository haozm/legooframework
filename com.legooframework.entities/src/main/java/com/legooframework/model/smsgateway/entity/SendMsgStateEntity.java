...
public class SendMsgStateEntity extends BaseEntity<String> implements BatchSetter {
    ...
    private SendMsgStateEntity(Integer companyId, Integer storeId, MsgEntity sms, boolean freeSend) {
        super(sms.getSmsId(), companyId.longValue(), -1L);
        this.sms = sms;
        if (this.sms.isEnbaled()) {
            this.sendStatus = SendStatus.Msg4Inited;
        } else {
            this.sendStatus = SendStatus.Msg4InitError;
        }
    ...
    public static SendMsgStateEntity createInstance(StoEntity store, MsgEntity sms, boolean freeSend) {
        return new SendMsgStateEntity(store.getCompanyId(), store.getId(), sms, freeSend);
    }

    public static SendMsgStateEntity createInstance(StoEntity store, MsgEntity sms) {
        return new SendMsgStateEntity(store.getCompanyId(), store.getId(), sms, false);
    }
    ...
    SendMsgStateEntity(String id, ResultSet res) {
        super(id, res);
        try {
